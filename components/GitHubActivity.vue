...
<template>
  <div class="flex justify-between items-center bg-neutral-1 dark:bg-neutral-8 rounded-xl p-3 overflow-clip">
    <div class="flex  items-center gap-4 px-2">
      <ClientOnly>
        <Icon :name="iconName" v-if="iconName" class="size-8 min-w-6 relative bg-neutral-5 mr-2" />
      </ClientOnly>
      <span class="hidden sm:block mr-2 text-gray-4">{{ activity.type }}</span>
      <a :href="getHtmlUrl(activity.repo.url)"
        class="underline underline-offset-6 underline-dashed underline-neutral-4 text-ellipsis ">{{ activity.repo.name
        }}</a>
    </div>
